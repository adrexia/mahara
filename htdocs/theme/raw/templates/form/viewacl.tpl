...
<tr id="row-{%=o.id%}" data-id="{%=o.id%}">
    <td class="text-center pr0 ptm tiny">
                ...
                <select id="type-{%=o.id%}" name="accesslist[{%=o.id%}][searchtype]" class="js-share-type form-control input-small select" {% if (o.presets.locked) { %}disabled{% } %}>
                    ...
                    <optgroup label="{%={{jstr tag=institutions section=view}}%}" id="potentialpresetitemsinstitutions">
                        {% for (var i=0; i<o.shareoptions.institutions.length; i++) { %}
                            <option data-type="institution" value="{%=o.shareoptions.institutions[i].id%}"{% if (o.presets.type == "institution" && o.presets.id == o.shareoptions.institutions[i].id) { %} selected{% } %}>{%=o.shareoptions.institutions[i].name%}</option>
                        {% } %}
                    </optgroup>
                    <optgroup label="{%={{jstr tag=groups section=view}}%}" id="potentialpresetitemsgroups">
                        {% for (var i=0; i<o.shareoptions.myGroups.length; i++) { %}
                            <option data-type="group" value="{%=o.shareoptions.myGroups[i].id%}"{% if (o.presets.type == "group" && o.presets.id == o.shareoptions.myGroups[i].id) { %} selected{% } %}>
    ...
    {% if (o.viewtype !== "profile") { %}
        <td class="text-center tiny">
            <input value="1" name="accesslist[{%=o.id%}][allowcomments]" class="mtm allow-comments-checkbox js-hide-empty {% if (o.presets.empty) { %}hidden{% } %}" type="checkbox" {% if (o.presets.allowcomments == "1") { %}checked{% } else if(o.presets.allowcomments == "0") { %}{% } else { %}checked{% } %} {% if (o.presets.locked) { %}disabled{% } %}>
        </td>
        <td class="text-center tiny">
            <input value="1" name="accesslist[{%=o.id%}][approvecomments]" class="mtm moderate-comments-checkbox js-hide-empty {% if (o.presets.empty) { %}hidden{% } %}" type="checkbox" {% if (o.presets.allowcomments == "1" && o.presets.approvecomments == "1") { %}checked{% } %}  {% if (o.presets.locked || o.presets.allowcomments == "0") { %}disabled{% } %}>
        </td>
    {% } %}
</tr>
</script>
